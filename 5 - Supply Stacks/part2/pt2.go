...
func (crane *CargoCrane) Rearrange(stacks []SupplyStack, instructions []Instruction) []SupplyStack {
	...
	*/
	...
	for _, instruction := range instructions {
		...
		fromStack = stackMap[instruction.from]
		toStack = stackMap[instruction.to]
		var cratesToMove []rune
		lengthOfFromStack := len(fromStack)

		cratesToMove, fromStack = fromStack[lengthOfFromStack-instruction.amount:lengthOfFromStack], fromStack[:lengthOfFromStack-instruction.amount]
